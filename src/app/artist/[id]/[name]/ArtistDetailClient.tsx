...
function formatPlaycount(n: number | null | undefined) {
  if (!n) return ''
...
export default function ArtistDetail() {
  ...
  async function navigateToSimilar(name: string) {
    try {
      ...
      // fallback: navigate without an ID — the page will still work if the artist
      // somehow ends up in DB by the time it loads
    }
  }
        ...
        {(artist?.spotifyAlbums?.length ?? 0) > 0 && !loading && (
          <div style={{ marginBottom: 40 }}>
            ...
            <div className="album-list">
              {artist!.spotifyAlbums.map(album => (
        ...
        {(artist?.lastfmTopTracks?.length ?? 0) > 0 && !loading && (
          <div style={{ marginBottom: 40 }}>
            ...
            <ul className="track-list" id="top-tracks-list">
              {artist!.lastfmTopTracks.map((t, i) => {
                const spotifySearchUrl = `https://open.spotify.com/search/${encodeURIComponent(displayName + ' ' + t.name)}`
                return (
