...
function parseJSONL(file: File): Promise<any[]> {
...
function normaliseEvents(eventRows: any[], lineupRows: any[]) {
  ...
  eventRows.forEach(item => {
    ...
    lineup = [...new Set(lineup.map((s: string) => String(s).trim()).filter(Boolean))]
...
export default function RAImport() {
  ...
  const [preview, setPreview] = useState<any>(null)   // { events, skipped } before committing
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      const alreadyPresent = 0 // Duplicates handled by DB upsert
          ...
          {preview && (
            <div style={{
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                {Object.values(preview.events).slice(0, 4).map((ev: any) => (
